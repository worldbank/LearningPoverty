...
program define _simulation_dataset, rclass
	...
	noi di
	noi di in r "ATTENTION: " in y "_simulation_dataset" in g " is pulling the data from " in y "`inputfolder'"
	noi di

	qui {

	di "preference: `preference'"
	clear
	...
	*********************************************************************
	...
	if ("`nosimulation'" == "") {
		...
		*Replace with values in markdown file if specified
		if ("`usefile'" != "") {
		cap drop _merge
		noi merge m:1 `groupingspells' using "`usefile2'", update replace
