qui {
  ...
  * Drop observations specified by [if] [in].
  if `"`ifspell'"'!=""  {
    di `"`ifspell'"'
    keep `ifspell'
  }
  ...
  * Identify only selected spells (n=71)
  ...
  keep if test != "no assessment" & test != "EGRA" & delta_adj_pct != . & delta_adj_pct > -2 & delta_adj_pct < 4 & test != "PASEC" & year_assessment>2000 & countrycode!="ISR"
...
*********************************************'
  ...
  foreach t of local tr {
    ...
    foreach r of local reg {
      ...
      count if !missing(delta_adj_pct ) & threshold == "`t'" & region == "`r'" & test != "EGRA" ///
               & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & countrycode!="ISR"
      local count=`r(N)'
      ...
      *Only make change to regional if we have at least 3 regional spells
      ...
      _pctile delta_adj_pct if threshold == "`t'" & region == "`r'" & test != "EGRA" ///
              & test != "PASEC" & test != "no assessment"  & delta_adj_pct > -2 & delta_adj_pct < 4 & year_assessment>2000 & countrycode!="ISR", ///
      ...
      _pctile delta_adj_pct  [aw = wtg] if threshold == "`t'" & region == "`r'" & test != "EGRA" ///
              & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & delta_adj_pct > -2 & delta_adj_pct < 4 & countrycode!="ISR" , ///
              percentiles(50(10)90)
      ...
      _pctile delta_adj_pct if threshold == "`t'" & region == "`r'" & test != "EGRA" ///
              & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & delta_adj_pct > -2 & delta_adj_pct < 4 & countrycode!="ISR"& test != "SACMEQ", ///
              percentiles(50(10)90)

      replace delta_reg_50_noSQ = r(r1) if threshold == "`t'" & region == "`r'" & test != "EGRA" ///
              & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & countrycode!="ISR"& test != "SACMEQ"
  ...
  ** with SACMEQ (no PASEC)
  tabstat delta_adj_pct  delta_reg_50 delta_reg_60 delta_reg_70 delta_reg_80 delta_reg_90 if test != "EGRA" ///
          & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & countrycode!="ISR", by( region )
  ...
  tabstat delta_adj_pct  delta_reg_50_noSQ delta_reg_60_noSQ delta_reg_70_noSQ delta_reg_80_noSQ delta_reg_90_noSQ if test != "EGRA" ///
          & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & countrycode!="ISR"& test != "SACMEQ", by( region ) stat(mean)


  ** No SAQMEC (no PASEC)
  tabstat delta_adj_pct delta_reg_w_50 delta_reg_w_60  delta_reg_w_70 delta_reg_w_80 delta_reg_w_90 if test != "EGRA" ///
          & test != "PASEC" & test != "no assessment"  & year_assessment>2000 & countrycode!="ISR", by( region ) stat(mean N)
