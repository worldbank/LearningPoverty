*==============================================================================*
* 0521 SUBTASK: EXPORT TO EXCEL WORKING PAPER TABLES AND FIGURES
*==============================================================================*
qui {
  ...
  * Table 18 Population and country coverage by country groups, latest available learning assessment
  *-----------------------------------------------------------------------------
  local cell_old_0 "C8"
  ...
  * N/A trick for NAC and Part1 in "Low and Middle Contries" panel
  ...
  noi disp as txt "Table 3 & Table 18 exported"


  *-----------------------------------------------------------------------------
  * Table 4 Assessment comparability in terms of grades by region and income level
  *-----------------------------------------------------------------------------
  ...
  export excel using "${excel_file}", sheet("T4", modify) cell(B7) firstrow(varlabels) nolabel keepcellfmt
  ...
  collapse (max) *_countries, by(countrycode idgrade)
  keep if rich_countries == 1 | client_countries == 1
  merge m:1 countrycode using "${clone}/01_data/013_outputs/rawlatest.dta", keepusing(population_2015_all) assert(match using) keep(match) nogen
  gen population_rich   = population_2015_all * rich_countries
  gen population_client = population_2015_all * client_countries
  ...
  export excel using "${excel_file}", sheet("T4", modify) cell(G7) firstrow(varlabels) nolabel keepcellfmt

  noi disp as txt "Table 4 exported"


  *-----------------------------------------------------------------------------
  * Table 5 Temporal comparability within assessments
  *-----------------------------------------------------------------------------
  use "${clone}/02_simulation/023_outputs/all_spells.dta", clear
  drop if test == "EGRA"
  ...
  export excel using "${excel_file}", sheet("T5", modify) cell(B8) nolabel keepcellfmt
  ...
  * List of outliers from Part2
  ...
  sort test delta_lp
  export excel using "${excel_file}", sheet("T5", modify) cell(B19) firstrow(variables) nolabel keepcellfmt

  noi disp as txt "Table 5 exported"


  *-----------------------------------------------------------------------------
  * Table 6 Share of children who are learning-poor by late primary by country groups
  *-----------------------------------------------------------------------------
  use "${clone}/03_export_tables/033_outputs/individual_tables/outline_all_current_lp.dta", clear
  ...
  keep  group - max_lp1
  replace group = "Overall" if group == "TOTAL"
  export excel using "${excel_file}", sheet("T6", modify) cell(C8) nolabel keepcellfmt

  * N/A trick for NAC and Part1 in "Low and Middle Contries" panel
  fill_na, ncol(4)
  export excel using "${excel_file}", sheet("T6", modify) cell(I13) nolabel keepcellfmt
  export excel using "${excel_file}", sheet("T6", modify) cell(I20) nolabel keepcellfmt

  noi disp as txt "Table 6 exported"


  *-----------------------------------------------------------------------------
  * Table 7 Results sensitivity in respect to choice of reporting window
  *-----------------------------------------------------------------------------
  ...
  export excel using "${excel_file}", sheet("T7", modify) cell(B7) firstrow(varlabels) nolabel keepcellfmt

  noi disp as txt "Table 7 exported"


  *-----------------------------------------------------------------------------
  * Table 8 Results sensitivity in respect to choice of population of reference
  *-----------------------------------------------------------------------------
  use "${clone}/03_export_tables/033_outputs/individual_tables/outline_all_SA_lp.dta", clear
  keep if aggregated_by == "global"
  ...
  export excel using "${excel_file}", sheet("T8", modify) cell(B7) firstrow(varlabels) nolabel keepcellfmt

  noi disp as txt "Table 8 exported"


  *-----------------------------------------------------------------------------
  * Table 9  Decomposition of learning poverty by learning and schooling
  *-----------------------------------------------------------------------------
  ...
  * N/A trick for NAC and Part1 in "Low and Middle Contries" panel
  fill_na, ncol(5)
  export excel using "${excel_file}", sheet("T9", modify) cell(J14) nolabel keepcellfmt
  export excel using "${excel_file}", sheet("T9", modify) cell(J21) nolabel keepcellfmt

  noi disp as txt "Table 9 exported"


  *-----------------------------------------------------------------------------
  * Table 10 Country Ranks on PASEC 2014/2015
  *-----------------------------------------------------------------------------
  use "${clone}/05_working_paper/053_outputs/pasec-rank-early-end.dta", clear
  * Add a blank column and a blank line
  ...
  drop m_earlyscoreread m_endscoreread m_earlyscoremath m_endscoremath m_endfgt0read m_endfgt1read m_endfgt2read
  drop fgt2_read_end
  order countryname score_read_early score_read_end blank1 score_math_early score_math_end score_math_end blank2

  export excel using "${excel_file}", sheet("T10", modify) cell(B9) nolabel keepcellfmt
  noi disp as txt "Table 10 exported"


  *-----------------------------------------------------------------------------
  * Table 11 Learning poverty by boys and girls, and country groups, for a subsample of countries
  *-----------------------------------------------------------------------------
  ...
  replace group = "Overall" if group == "TOTAL"
  export excel using "${excel_file}", sheet("T11", modify) cell(C9) nolabel keepcellfmt

  * N/A trick for NAC and Part1 in "Low and Middle Contries" panel
  fill_na, ncol(5)
  export excel using "${excel_file}", sheet("T11", modify) cell(J14) nolabel keepcellfmt
  export excel using "${excel_file}", sheet("T11", modify) cell(J21) nolabel keepcellfmt
  ...
  * - list of Enrollment gender flag countries
  ...
  export excel using "${excel_file}", sheet("T11", modify) cell(B29) nolabel keepcellfmt
  ...
  * - share of population with gender disaggregated data
  ...
  export excel using "${excel_file}", sheet("T11", modify) cell(J29) firstrow(variables) nolabel keepcellfmt
  use "${clone}/01_data/013_outputs/preference1005.dta", clear
  ...
  export excel using "${excel_file}", sheet("T11", modify) cell(J34) firstrow(variables) nolabel keepcellfmt

  noi disp as txt "Table 11 exported"


  *-----------------------------------------------------------------------------
  * Table 12  Decomposition of learning poverty by learning and schooling, for boys and girls
  *-----------------------------------------------------------------------------
  * Gender disaggregated, All countries only
  ...
  order category total0 bmp0 oos0 shr_bmp0 shr_oos0 blank total1 bmp1 oos1 shr_bmp1 shr_oos1
  keep  category - shr_oos1
  export excel using "${excel_file}", sheet("T12", modify) cell(C9) nolabel keepcellfmt

  noi disp as txt "Table 12 exported"
  ...
  *-----------------------------------------------------------------------------
  * Table 14 Annualized change in learning poverty (in percentage points) by
  ...
  *-----------------------------------------------------------------------------
  * Table 15 Learning poverty rates in 2030 under two scenarios (simulation using spells by region)
  *-----------------------------------------------------------------------------
  local table_15_A_file "simfile_preference_1005_regional_growth_summarytable.dta"
  local table_15_B_file "simfile_preference_1005_income_level_summarytable.dta"
  local table_15_C_file "simfile_preference_1005_initial_poverty_level_summarytable.dta"
  local table_15_D_file "simfile_preference_1005_regional_growth_glossy_summarytable.dta"
  local table_15_E_file "simfile_preference_1005_regional_growth_min2_summarytable.dta"
  local table_15_A_place "B9"
  local table_15_B_place "B25"
  local table_15_C_place "B37"
  local table_15_D_place "B51"
  local table_15_E_place "B63"

  * Run for each panel in Table 15
  foreach table in table_15_A table_15_B table_15_C table_15_D table_15_E {
  ...
  noi disp as txt "Table 15 exported"


  *-----------------------------------------------------------------------------
  * Table 16 Relationship between reading and math proficiency (PISA)
  *-----------------------------------------------------------------------------
  ...
  drop if _n == _N
  export excel using "${excel_file}", sheet("T16", modify) cell(C8) nolabel keepcellfmt

  noi disp as txt "Table 16 exported"


  *-----------------------------------------------------------------------------
  * Table 17 Non-comparable spells in PIRLS and TIMMS
  *-----------------------------------------------------------------------------
  ...
  export excel using "${excel_file}", sheet("T17", modify) cell(G6) firstrow(variables) nolabel keepcellfmt

  noi disp as txt "Table 17 exported"


  *-----------------------------------------------------------------------------
  * Table 19  Source of enrollment data
  *-----------------------------------------------------------------------------
  use "${clone}/01_data/013_outputs/preference1005.dta", clear
  ...
  preserve
    ...
    export excel using "${excel_file}", sheet("T19", modify) cell(B6) firstrow(varlabels) nolabel keepcellfmt
  restore
  ...
  export excel using "${excel_file}", sheet("T19", modify) cell(F6) firstrow(varlabels) nolabel keepcellfmt

  noi disp as txt "Table 19 exported"


  *-----------------------------------------------------------------------------
  * Table 20 Population ages 10-14 years old by region and income classifications (Year = 2015)
  *-----------------------------------------------------------------------------
  use "${clone}/01_data/013_outputs/preference1005.dta", clear
  ...
  *-----------------------------------------------------------------------------
  * Table 21  Weighted average and correlation of PISA and Learning Poverty country
  ...
  * Add a blank column and a blank line
  ...
  sort obs_n
  export excel using "${excel_file}", sheet("T21", modify) cell(E8) nolabel keepcellfmt

  noi disp as txt "Table 21 exported"


  *-----------------------------------------------------------------------------
  * Table 22 Correlation of early grade and end of primary scores
  *-----------------------------------------------------------------------------
  ...
  * Add a blank column and a blank line
  ...
  keep  Early_Grade-N
  export excel using "${excel_file}", sheet("T22", modify) cell(D7) nolabel keepcellfmt

  noi disp as txt "Table 22 exported"


  *-----------------------------------------------------------------------------
  * Table 23 Country Numbers
  *-----------------------------------------------------------------------------
  use "${clone}/01_data/013_outputs/preference1005.dta", clear
  ...
  * and COD also had the actual year of 2010 disguised as 2011
  ...
  export excel using "${excel_file}", sheet("T23", modify) cell(B6) firstrow(varlabels) nolabel keepcellfmt

  noi disp as txt "Table 23 exported"


  *-----------------------------------------------------------------------------
  * Table 24 Summary statistics of the annualized changes in learning poverty
  ...
  * Add a blank column and a blank line
  ...
  replace obs_n = 6.5 if missing(obs_n)
  sort obs_n
  export excel using "${excel_file}", sheet("T24", modify) cell(C8) nolabel keepcellfmt

  noi disp as txt "Table 24 exported"


  *-----------------------------------------------------------------------------
  * Table 25  Summary statistics of the annualized changes in learning poverty and
